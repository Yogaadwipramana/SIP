      ...
      if($this->model_user_access->access_access() == 1){
        ...
        if(!empty($tgl_awal)){
          ...
          $where      = array('id_group' => $id_group);
            ...
            $data_estimasi = $this->db->query("SELECT * FROM barang_keluar_estimasi WHERE id_group = '$id_group' GROUP BY no_transaksi")->result();
            $data_realisasi = $this->db->query("SELECT * FROM barang_keluar_realisasi WHERE id_group = '$id_group'  GROUP BY no_transaksi")->result();
          // $data_real  = $this->model_global->edit_data($where,'barang_keluar')->result();
        }

        // $count_estimasi = count($data_estimasi);

        // echo "Jumlah data estimasi: " . $count_estimasi;
  ...
  public function add_action_get(){
    if($this->session->userdata('name_user') and $this->session->userdata('username')){
      if($this->model_user_access->access_add() == 1){
            ...
            $keterangan2        = addslashes($this->input->post('id_barang_real'));
        ...
        die();*/
                ...
                if($jenis_keluar == 3){
                      ...
                      for ($i=0; $i < $limit_pk; $i++) {
                          ...
                          $qty        = addslashes($this->input->post('qty' . $i));
                          $sat        = $sat_real;
                          $harga1     = addslashes($this->input->post('harga' . $i));
            ...
            $count_loop = addslashes($this->input->post('count_loop'));
                ...
                if($jenis_keluar == 3){
                      ...
                      for ($i = 1; $i <= $count_loop; $i++) {
                          ...
                          $sat        = $sat_real;
                          $harga2     = addslashes($this->input->post('harga_satuan_real' . $i));
                          $harga1     = str_replace(".","",$harga2);
                          $harga      = str_replace(",",".",$harga1);
                          $total2     = addslashes($this->input->post('hasil_real' . $i));
                          $total1     = str_replace(".","",$total2);
