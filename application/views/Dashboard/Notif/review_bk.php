                ...
                <table width="100%" style="margin-top: 10px;" class="table table-striped">
                  ...
                  <tbody id="myTablePK">
                    ...
                    foreach ($data_realisasi_result as $value_result) :
                      ...
                      // Query untuk mendapatkan jumlah estimasi
                      $get_estimasi = $this->db->query("SELECT jumlah FROM barang_keluar_estimasi WHERE no_transaksi = '$value_result->no_transaksi' AND terminal_tank = '$value_result->terminal_tank'")->row();

                      // Query untuk mendapatkan jumlah realisasi
                      $get_realisasi = $this->db->query("SELECT jumlah FROM barang_keluar_realisasi WHERE no_transaksi = '$value_result->no_transaksi' AND terminal_tank = '$value_result->terminal_tank'")->row();

                      // Hitung selisih, default adalah 0 jika salah satu data tidak ditemukan
                      $hasil_hitung = 0;
                      if ($get_estimasi && $get_realisasi) {
                        $hasil_hitung = $get_estimasi->jumlah - $get_realisasi->jumlah;
                      }

                      // Jumlah total untuk looping
                      $sumTotalLoopQty += $value_result->jumlah;
                      $sumTotalLoopHarga += $value_result->harga;
                      $sumTotalLoopTH += $value_result->nilai_barang;
                      $sumTotalLoopBK += $value_result->biaya_kurs;
